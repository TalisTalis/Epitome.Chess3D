...
{
    [Header("Art stuff")]
    [SerializeField] private Material tileMaterail;
    ...
    private void Start()
    {
        ...
        GenerateAllTile(tileSize, TILE_COUNT_X, TILE_COUNT_Y);
        SpawnAllPieces();
        PositionAllPieces();
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(ray, out info, 100, LayerMask.GetMask("Tile", "Hover", "Highlight")) && GameStart)
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                {
                    //Debug.Log("// If we press down on the mouse black " + (currentTeam));
                        ...
                        || (chessPieces[hitPosition.x, hitPosition.y].team == 1 && !IsWhiteTurn && currentTeam == 1))
                    {
                        ...
                        // Get a list of special moves well
            ...
            if (currentlyDragging != null && Input.GetMouseButtonUp(0))
            {
                ...
                if (ContainsValidMove(ref availableMoves, new Vector2Int(hitPosition.x, hitPosition.y)))
                {
                    ...
                    //Debug.Log("// Net implementation " + (currentTeam));
                }
                else
                {
                    ...
                    //Debug.Log("ELSE // Net implementation " + (currentTeam));
                }
                currentlyDragging = null;
                RemoveHighlightTiles();
                //Debug.Log("// If we are releasing the mouse button " + (currentTeam));
            }
        }
        else
        {
    ...
    // Ganerate the board
    private void GenerateAllTile(float tileSize, int tileCountx, int tileCountY)
    {
    ...
    private GameObject GenerateSingleTile(float tileSize, int x, int y)
    {
        ...
        tileObject.AddComponent<MeshRenderer>().material = tileMaterail;

        Vector3[] verticals = new Vector3[4];
        verticals[0] = new Vector3(x * tileSize, yOffset, y * tileSize) - bounds;
        verticals[1] = new Vector3(x * tileSize, yOffset, (y + 1) * tileSize) - bounds;
        verticals[2] = new Vector3((x + 1) * tileSize, yOffset, y * tileSize) - bounds;
        verticals[3] = new Vector3((x + 1) * tileSize, yOffset, (y + 1) * tileSize) - bounds;
    ...
    private ChessPiece SpawnSinglePiece(ChessPieceType type, int team)
    {
        ...
        if (cp.type == ChessPieceType.Knight && cp.team == 0)
        {
            cp.transform.rotation = new Quaternion(cp.transform.rotation.x, 180f, cp.transform.rotation.z, 0f);
        }
    ...
    private void DisplayVictory(int winningTeam)
    {
        ...
        //victoryScreen.transform.GetChild(2).gameObject.SetActive(!IsWin);
    }
    ...
    private void SimulateMoveForSinglePiece(ChessPiece cp, ref List<Vector2Int> moves, ChessPiece targetKing)
    {
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            // Did ine of the piece got taken down during our simulation
    ...
    private bool CheckForCheckmate()
    {
        ...
        int targettTeam = (chessPieces[lastMove[1].x, lastMove[1].y].team == 0) ? 1 : 0;
        ...
        for (int x = 0; x < TILE_COUNT_X; x++)
        {
            for (int y = 0; y < TILE_COUNT_Y; y++)
            {
                if (chessPieces[x, y] != null)
                {
                    if (chessPieces[x, y].team == targettTeam)
                    {
        ...
        {
            // Kingid=s under attack, can we move something to help him?
            for (int i = 0; i < defendingPieces.Count; i++)
            {
                List<Vector2Int> defendingMoves = defendingPieces[i].GetAvailableMoves(ref chessPieces, TILE_COUNT_X, TILE_COUNT_Y);
                // Since we're sending ref availableMoves, we will be deleting move that are putting us in check
    ...
    {
        // Client has connected, assign a team and the message back to him
    ...
    private void OnMakeMoveClient(NetMessage msg)
    {
        ...
        //Debug.Log("OnMakeMoveClient currentTeam: " + currentTeam);
        if (mm.teamId != currentTeam)
        {
            //Debug.Log("OnMakeMoveClient mm.teamId != currentTeam currentTeam: " + currentTeam);
    ...
    private void OnRematchClient(NetMessage msg)
    {
        ...
        //Debug.Log("rm.wantRematch "+ rm.wantRematch);
        //Debug.Log("rm.teamId != currentTeam " + (rm.teamId != currentTeam));

        // Activate the piece of UI
        if (rm.teamId != currentTeam)
        {
            Debug.Log("wantRematch " + rm.wantRematch);
